...
void IMUSensor::parse_data(float* array,  //< [INPUT] address of the first element in an array 3
                                          // elements long. elements 0-3 will be RPY
...
void IMUSensor::MAG_request(float* array, serialib serial, byte* buffer) {
    //    uint16 toId = ui->spinBox_TX_Request_ShortId->value();     // Step 1: Get the destination
    //    device ID to which the request is sent
    uint16 toId = EP_ID_BROADCAST_;  //       You can also choose to boardcast the request without
                                     //       specifying the target device ID
...
// void IMU_RPY(float *array,int port, byte *buffer)// *array is a 3 long array, 0 is roll 1 is
// pitch 2 is yaw, port is which port the IMU is connected to
// {
// 		serialib serial;
// 		char errorOpening = serial.openDevice(SERIAL_PORT, 115200);
// 		// If connection fails, return the error code otherwise, display a success message
...
//     uint16 toId = EP_ID_BROADCAST_;  //       You can also choose to boardcast the request
//     without
//                                      //       specifying the target device ID
//     if (EP_SUCC_ ==
//         eOD.Write_Ep_Request(toId,
//                              EP_CMD_RPY_)) {  // Step 2: Write the device ID into the data
//                              structure
//                                               // (i.e. the request itself) pending to be sent
//         EP_ID_TYPE_ txToId;
//         char* txPkgData;
...
//                                       &txPkgSize)) {  // Step 4: create a package out of the data
//                                                       // structure (i.e. the payload) to be sent
//             serial.writeBytes(txPkgData, txPkgSize);  // Step 5:  Send the package via Serial
//             Port
//         }
//     }
...
//                                    // It does not matter if the new data only contains a fraction
//                                    // of a complete data package, nor does it matter if the data
//                                    // stream is corrupted during the transmission. On_RecvPkg()
//                                    will
//                                    // only return EP_SUCC_ when a complete and correct package
//                                    has
//                                    // arrived. Example Reading of the Short ID of the device who
//                                    // sends the data:
//         uint32 fromId =
...
//                 if (EP_SUCC_ ==
//                     eOD.Read_Ep_RPY(
//                         &ep_RPY)) {  // Another Example reading of the received Roll Pitch and
//                         Yaw
//                     float roll = ep_RPY.roll;
//                     float pitch = ep_RPY.pitch;
//                     float yaw = ep_RPY.yaw;
