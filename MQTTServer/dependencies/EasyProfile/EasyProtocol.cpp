...
int EasyProtocol::Checksum_Verify(
    ...
    int dataLength,              ///< [INPUT]
    uint16 checksumToBeVerified  ///< [INPUT] Depending on context, use the lower 8-bit or the
                                 ///< entire 16-bit
) {
...
int EasyProtocol::UnWrapInData(char* packageData,   ///< [INPUT]
    ...
    if (CHECKSUM_LENGTH_ == 1) {
        ...
        recvChecksum =
            *((uint16*)(packageData + payloadSize + EP_PKG_MODIFIER_SIZE_ - CHECKSUM_LENGTH_));
    }
    ...
    if (EP_SUCC_ !=
        Checksum_Verify(packageData + HEAD_LENGTH_, payloadSize + SIZE_LENGTH_, recvChecksum)) {
#ifdef EP_TURN_ON_STATISTICS_
        sRecvPkg_Bad++;
#endif
        *payloadData = 0;
...
int EasyProtocol::AssembleInputPackage(
    ...
    char** payloadData,  ///< [OUTPUT] Return a pointer to the payload of the successfuly received
                         ///< package.
                         ///              The memory is allocated within this EasyProtocol object.
                         ///              If no package is received successfully, it is set to 0.
    int* payloadSize     ///< [OUTPUT] Return the size of the payload of the successfuly received
                         ///< package.
                         ///              If no package is received successfully, it is set to 0.
) {
            ...
            if (p >= (inBuf + HEAD_LENGTH_ + SIZE_LENGTH_)) {
                ...
                if (ENABLE_ROUND_UP_) {
                    if ((int32)declaredPayloadSize % ((int32)ROUND_UP_NUM_)) {
                        roundUpTmp = ((int32)ROUND_UP_NUM_) -
                                     ((int32)declaredPayloadSize % ((int32)ROUND_UP_NUM_));
                    }
                }
#ifdef EP_TURN_ON_DEBUG_
