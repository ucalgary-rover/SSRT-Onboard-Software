 ...
 *         EP_FAIL_            lengthIn doesn't match the type size, or the cmd does not exist in
 * the object dictionary EP_MUTEX_LOCKED_    Permission denied while trying to access the Object.
 */
...
int EasyObjectDictionary::Read_Header(
    ...
    // Enable ReadProtection:
    // EOD_DB_SetReadProtect(cmdIn, true); // Same effect as below, but has lower efficiency in
    // execution
    eOD_DB_Dynamic[key].mutex |= EOD_MUTEX_READ_PROTECT_;
    ...
    // EOD_DB_SetReadProtect(cmdIn, false); // Same effect as below, but has lower efficiency in
    // execution
    eOD_DB_Dynamic[key].mutex &= (~EOD_MUTEX_READ_PROTECT_);
    return EP_SUCC_;
}
...
//------------------------------------------------------------------------
/**
 * Copy data in a byte array into the relevant structure specified by the cmd at the beginning of
 * the byte array.
 * @return EP_SUCC_            Write successfully
 *         EP_FAIL_            lengthIn doesn't match the type size, or the cmd does not exist in
 * the object dictionary EP_MUTEX_LOCKED_    Write permission denied.
 */
