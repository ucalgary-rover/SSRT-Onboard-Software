...
void RPY_request(float *array, serialib serial)
{
//    uint16 toId = ui->spinBox_TX_Request_ShortId->value();     // Step 1: Get the destination device ID to which the request is sent
	...
	std::byte buffer[1];
    	std::byte header1 = buffer[0];
    	serial.readBytes(buffer,1,200,1);
    	std::byte header2 = buffer[0];
    	serial.readBytes(buffer,1,200,1);
    	int length = buffer[0];
...
void IMU_RPY(float *array,int port)// *array is a 3 long array, 0 is roll 1 is pitch 2 is yaw, port is which port the IMU is connected to
{
		serialib serial;
